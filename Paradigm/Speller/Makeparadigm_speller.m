...
for ch=1:length(test_character)
    flickering_order(:,:,ch)=Shuffle(repmat([1:12]',[1,10])); % �� ���ڸ��� 120���� flickering
end
for fk=1:length(flickering_order(:))
    if flickering_order(fk)<=6      % 1~6���� ��
...
for cur_char = 1:length(test_character)
    ...
    %% target���� �����ֱ�
    normal_layout(w, test_character_show, spell_char, loc_layout, n_text_size); %smkim(�߰�)
    Screen('TextFont',w, 'Arial');
...
%                 run=false;
        ...
        if run_p300
            ...
            if(p300_interval >=0.2)   % p300 isi
                ...
                target_on=true;
                a = ismember(cell_order_all(count_run,:), target_ind); %% Ÿ�ٺκ��ϰ�츸 trigger
                if sum(a)       % target
%                     % ppWrite(IO_ADD,1);
                else            % non-target
%                     % ppWrite(IO_ADD,2);
            ...
            if target_on
                ...
                if p300_sti<0.09 % 0.09���ȸ� Ÿ���� ��������  **   p300 sti
                    for j = 1:6 % target
